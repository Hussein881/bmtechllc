...
- `benchmark-qa` answers a question from one named document.
- `benchmark-agent` searches across all documents in the library and can combine information from multiple sources.

Example:

```bash
benchmark-agent --question "What p99 retrieval latency did pgvector achieve with HNSW?"
```
